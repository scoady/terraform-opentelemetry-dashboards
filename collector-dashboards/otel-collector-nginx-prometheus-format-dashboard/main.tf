...
resource "lightstep_metric_dashboard" "otel_collector_nginxreceiver_dashboard" {
    ...
    chart {
      name = "nginx_connections_current"
      rank = "1"
      type = "timeseries"
      ...
      query {
        ...
        metric              = "nginx_connections_current"
    ...
    chart {
      ...
      query {
        ...
        # TODO: add description: The total number of handled connections. Generally, the parameter value is the same as nginx_connections_accepted unless some resource limits have been reached (for example, the worker_connections limit).
        # TODO: add unit: connections
      }
    }
